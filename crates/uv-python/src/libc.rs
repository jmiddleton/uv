...
pub enum LibcDetectionError {
    ...
    #[error(
        "Failed to determine libc version from common binaries ({0}). Are you running uv from a distroless Docker image or similar? That's not currently supported."
    )]
